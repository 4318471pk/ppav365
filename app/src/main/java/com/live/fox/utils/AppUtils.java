...
import com.live.fox.utils.utilconstants.MyLifecycleHandler;
...
/**
 isAppForeground         : 判断App是否处于前台 但需要多加代码 具体代码看方法注释
 installApk              : 安装 App（支持 8.0）需要配置一些其他信息 见https://www.jianshu.com/p/51a7c89cc854
...
public final class AppUtils {
    ...
    /**My Add-----------------------------------------------------------------------------------*/
    //  1.需要再Application的onCreate中加registerActivityLifecycleCallbacks(new MyLifecycleHandler());
    //  2.才可调用AppUtils.isAppInForeground()
    public static boolean isAppForeground() {
        return MyLifecycleHandler.isApplicationInForeground() ;
    }
    public static String getUserAgent(){
        return "qyzb110__"+getAppVersionName()+"_"+ DeviceUtils.getModel()+"_"+DeviceUtils.getSDKVersionName();
    }
    /**My Add-----------------------------------------------------------------------------------*/

    //安装APK 解决24版本上安装不上的问题 使用此方法需要配置一些信息
    public static void installApk(Context context, File apkFilePath) {
        if (apkFilePath == null || !apkFilePath.exists() || !apkFilePath.isFile()) {
            ToastUtils.showShort("tệp apk không tồn tại");
            return;
        }
        if (apkFilePath.toString().endsWith(".apk")) {
            ...
            ToastUtils.showShort("định dạng tệp apk là sai");
        }
    }
