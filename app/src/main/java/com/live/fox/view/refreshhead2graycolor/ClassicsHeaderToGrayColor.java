...
public class ClassicsHeaderToGrayColor extends LinearLayout implements RefreshHeader {

    private TextView mHeaderText;//标题文本
    private ImageView mArrowView;//下拉箭头
    private ImageView mProgressView;//刷新动画视图

    private ProgressDrawableToGrayColor mProgressDrawable;//刷新动画

    public ClassicsHeaderToGrayColor(Context context) {
        this(context, null);
    }

    public ClassicsHeaderToGrayColor(Context context, AttributeSet attrs) {
        super(context, attrs, 0);
        setGravity(Gravity.CENTER);
        mHeaderText = new TextView(context);
        mHeaderText.setTextColor(Color.GRAY);
        mProgressDrawable = new ProgressDrawableToGrayColor();
        mArrowView = new ImageView(context);
        mProgressView = new ImageView(context);
        mProgressView.setImageDrawable(mProgressDrawable);
        mArrowView.setImageDrawable(new ArrowDrawableToGrayColor());
