 ...
 * 高仿微信的图片加载进度等待框
 * 使用方法
 *   <me.bzcoder.easyglide.progress.WxPhotoCircleProgressView
...
public class WxPhotoCircleProgressView extends ProgressBar {
    ...
    private int mReachBarSize = DeviceUtils.dp2px(getContext(), 2); // 未完成进度条大小
    private int mNormalBarSize = DeviceUtils.dp2px(getContext(), 2); // 未完成进度条大小
    private int mReachBarColor = Color.parseColor("#108ee9"); // 已完成进度颜色
    private int mNormalBarColor = Color.parseColor("#FFD3D6DA"); // 未完成进度颜色
    private int mTextSize = DeviceUtils.sp2px(getContext(), 14); // 进度值字体大小
    ...
    private int mRadius = DeviceUtils.dp2px(getContext(), 20); // 半径
    private int mStartArc; // 起始角度
    private int mInnerBackgroundColor; // 内部背景填充颜色
    private int mProgressStyle = ProgressStyle.NORMAL; // 进度风格
    private int mInnerPadding = DeviceUtils.dp2px(getContext(), 1); // 内部圆与外部圆间距
    ...
    private int mOuterSize = DeviceUtils.dp2px(getContext(), 1); // 外层圆环宽度
    ...
    public WxPhotoCircleProgressView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public WxPhotoCircleProgressView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void obtainAttributes(AttributeSet attrs) {
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.WxPhotoCircleProgressView);
        mProgressStyle = ta.getInt(R.styleable.WxPhotoCircleProgressView_cpv_progressStyle, ProgressStyle.NORMAL);
        // 获取三种风格通用的属性
        mNormalBarSize = (int) ta.getDimension(R.styleable.WxPhotoCircleProgressView_cpv_progressNormalSize, mNormalBarSize);
        mNormalBarColor = ta.getColor(R.styleable.WxPhotoCircleProgressView_cpv_progressNormalColor, mNormalBarColor);

        mReachBarSize = (int) ta.getDimension(R.styleable.WxPhotoCircleProgressView_cpv_progressReachSize, mReachBarSize);
        mReachBarColor = ta.getColor(R.styleable.WxPhotoCircleProgressView_cpv_progressReachColor, mReachBarColor);

        mTextSize = (int) ta.getDimension(R.styleable.WxPhotoCircleProgressView_cpv_progressTextSize, mTextSize);
        mTextColor = ta.getColor(R.styleable.WxPhotoCircleProgressView_cpv_progressTextColor, mTextColor);
        mTextSkewX = ta.getDimension(R.styleable.WxPhotoCircleProgressView_cpv_progressTextSkewX, 0);
        if (ta.hasValue(R.styleable.WxPhotoCircleProgressView_cpv_progressTextSuffix)) {
            mTextSuffix = ta.getString(R.styleable.WxPhotoCircleProgressView_cpv_progressTextSuffix);
        }
        if (ta.hasValue(R.styleable.WxPhotoCircleProgressView_cpv_progressTextPrefix)) {
            mTextPrefix = ta.getString(R.styleable.WxPhotoCircleProgressView_cpv_progressTextPrefix);
        }
        mTextVisible = ta.getBoolean(R.styleable.WxPhotoCircleProgressView_cpv_progressTextVisible, mTextVisible);

        mRadius = (int) ta.getDimension(R.styleable.WxPhotoCircleProgressView_cpv_radius, mRadius);
        ...
        switch (mProgressStyle) {
            ...
            case ProgressStyle.FILL_IN_ARC:
                mStartArc = ta.getInt(R.styleable.WxPhotoCircleProgressView_cpv_progressStartArc, 0) + 270;
                mInnerPadding = (int) ta.getDimension(R.styleable.WxPhotoCircleProgressView_cpv_innerPadding, mInnerPadding);
                mOuterColor = ta.getColor(R.styleable.WxPhotoCircleProgressView_cpv_outerColor, mReachBarColor);
                mOuterSize = (int) ta.getDimension(R.styleable.WxPhotoCircleProgressView_cpv_outerSize, mOuterSize);
                mReachBarSize = 0;// 将画笔大小重置为0
                mNormalBarSize = 0;
                if (!ta.hasValue(R.styleable.WxPhotoCircleProgressView_cpv_progressNormalColor)) {
            ...
            case ProgressStyle.NORMAL:
                mReachCapRound = ta.getBoolean(R.styleable.WxPhotoCircleProgressView_cpv_reachCapRound, true);
                mStartArc = ta.getInt(R.styleable.WxPhotoCircleProgressView_cpv_progressStartArc, 0) + 270;
                if (ta.hasValue(R.styleable.WxPhotoCircleProgressView_cpv_innerBackgroundColor)) {
                    mInnerBackgroundColor = ta.getColor(R.styleable.WxPhotoCircleProgressView_cpv_innerBackgroundColor, Color.argb(0, 0, 0, 0));
    ...
    public void setReachBarSize(int reachBarSize) {
        mReachBarSize = DeviceUtils.dp2px(getContext(), reachBarSize);
        invalidate();
    }
    ...
    public void setNormalBarSize(int normalBarSize) {
        mNormalBarSize = DeviceUtils.dp2px(getContext(), normalBarSize);
        invalidate();
    }
    ...
    public void setTextSize(int textSize) {
        mTextSize = DeviceUtils.sp2px(getContext(), textSize);
        invalidate();
    }
    ...
    public void setRadius(int radius) {
        mRadius = DeviceUtils.dp2px(getContext(), radius);
        invalidate();
    }
    ...
    public void setInnerPadding(int innerPadding) {
        mInnerPadding = DeviceUtils.dp2px(getContext(), innerPadding);
        int mInnerRadius = mRadius - mOuterSize / 2 - mInnerPadding;
        rectInner = new RectF(-mInnerRadius, -mInnerRadius, mInnerRadius, mInnerRadius);
        invalidate();
    }
    ...
    public void setOuterSize(int outerSize) {
        mOuterSize = DeviceUtils.dp2px(getContext(), outerSize);
        invalidate();
    }
