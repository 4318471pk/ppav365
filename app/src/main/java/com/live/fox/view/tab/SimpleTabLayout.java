...
/**
 * 导航栏tab
 * 在FlycoTabLayout上进行扩展 除拥有基于FlycoTabLayout的基本功能外，还带有设置选中文字大小的功能
 */
public class SimpleTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private float mTabTopPadding;
    private float mTabBottomPadding;
    private float mTabLeftMargin;
    private float mTabRightMargin;
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mTextSize = ta.getDimension(R.styleable.SlidingTabLayout_tl_textsize, normalTextSize);
        ...
        ta = context.obtainStyledAttributes(attrs, R.styleable.SimpleTabLayout);
        mTabTopPadding = ta.getDimension(R.styleable.SimpleTabLayout_tl_tab_top_padding, dp2px(0f));
        mTabBottomPadding = ta.getDimension(R.styleable.SimpleTabLayout_tl_tab_bottom_padding, dp2px(0f));
        mTabLeftMargin = ta.getDimension(R.styleable.SimpleTabLayout_tl_tab_left_margin, dp2px(0f));
        mTabRightMargin = ta.getDimension(R.styleable.SimpleTabLayout_tl_tab_right_margin, dp2px(0f));
        mTabSelectedBackground = ta.getDrawable(R.styleable.SimpleTabLayout_tl_tab_selected_background);
        mTabUnSelectedBackground = ta.getDrawable(R.styleable.SimpleTabLayout_tl_tab_un_selected_background);
        mSelectedTextSize = ta.getDimension(R.styleable.SimpleTabLayout_tl_selected_textsize, 0);
        mSelectedTextSize = mSelectedTextSize == 0 ? mTextSize : mSelectedTextSize;
        ta.recycle();
    }
     ...
     */
    public void setViewPager(ViewPager vp, ArrayList<CustomTabEntity> titles) {
        if (vp == null || vp.getAdapter() == null) {
            throw new IllegalStateException("ViewPager or ViewPager adapter can not be NULL !");
        }
    ...
    public void notifyDataSetChanged() {
        ...
        this.mTabCount = mViewPager == null ? mTabEntitys.size() : mViewPager.getAdapter().getCount();
        View tabView;
        for (int i = 0; i < mTabCount; i++) {
            tabView = View.inflate(mContext, R.layout.layout_tab, null);
            CharSequence pageTitle = mViewPager == null ? mTabEntitys.get(i).getTabTitle() : mViewPager.getAdapter().getPageTitle(i);
    ...
    private void updateTabStyles() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            if (tv_tab_title != null) {
                ...
                tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, isSelected ? mSelectedTextSize : mTextSize);
                // 增加 select 属性
                tv_tab_title.setPadding((int) mTabPadding, (int) mTabTopPadding, (int) mTabPadding, (int) mTabBottomPadding);
                if (mTabSelectedBackground != null && mTabUnSelectedBackground != null) {
                    tv_tab_title.setBackground(isSelected ? mTabSelectedBackground : mTabUnSelectedBackground);
                }
                RelativeLayout.LayoutParams lp_tab = (RelativeLayout.LayoutParams) tv_tab_title.getLayoutParams();
                lp_tab.leftMargin = i == 0 ? (int) (mTabLeftMargin * 2) : (int) mTabLeftMargin;
                lp_tab.rightMargin = i == mTabCount - 1 ? (int) (mTabRightMargin * 2) : (int) mTabRightMargin;
                tv_tab_title.setLayoutParams(lp_tab);
                ...
                } else if (isSelected && mTextBold == TEXT_BOLD_WHEN_SELECT) {
                    tv_tab_title.getPaint().setFakeBoldText(true);
                }
    ...
    private void scrollToCurrentTab() {
        if (mTabCount <= 0 || mCurrentTab > mTabsContainer.getChildCount() - 1) {
            return;
        }
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tab_title != null) {
                ...
                if (mTabSelectedBackground != null && mTabUnSelectedBackground != null) {
                    tab_title.setBackground(isSelect ? mTabSelectedBackground : mTabUnSelectedBackground);
                }
    ...
    public Fragment getCurrentTabFragment() {
        if (mFragmentChangeManager != null) {
            return mFragmentChangeManager.getCurrentFragment();
        }
        return null;
    }
    ...
    public Fragment getTabFragment(int pos) {
        if (mFragmentChangeManager != null) {
            return mFragmentChangeManager.getFragment(pos);
        }
        return null;
    }
