...
public class MarqueeView extends AppCompatTextView {
    ...
    private int DEFAULT_VERTICAL_INTERVAL = 1500;
    private int DEFAULT_HORIZONTAL_SPEED = 800;     //滚动每个字的时间
    ...
    ValueAnimator horizontalScrollAnimator1;
    ValueAnimator verticalSwitchAnimator;
    ValueAnimator verticalIntervalAnimator;

    ValueAnimator horizontalScrollAnimator2;
//    private int textPos = 0; //0,中间  1，上方  -1，下方
    ...
    public void onDestroyActivity() {
        if (horizontalScrollAnimator1 != null) {
            horizontalScrollAnimator1.removeAllListeners();
            horizontalScrollAnimator1.removeAllUpdateListeners();
            horizontalScrollAnimator1.cancel();
        }

        if (horizontalScrollAnimator2 != null) {
            horizontalScrollAnimator2.removeAllListeners();
            horizontalScrollAnimator2.removeAllUpdateListeners();
            horizontalScrollAnimator2.cancel();
        }

        if (verticalSwitchAnimator != null) {
            verticalSwitchAnimator.removeAllListeners();
            verticalSwitchAnimator.removeAllUpdateListeners();
            verticalSwitchAnimator.cancel();
        }

        if (verticalIntervalAnimator != null) {
            verticalIntervalAnimator.removeAllListeners();
            verticalIntervalAnimator.removeAllUpdateListeners();
    ...
    private void initAttrs(AttributeSet attrs) {
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.MarqueeView);
        verticalSwitchSpeed = array.getInt(R.styleable.MarqueeView_vertical_switch_speed, DEFAULT_VERTICAL_SPEED);
        verticalSwitchInterval = array.getInt(R.styleable.MarqueeView_vertical_switch_interval, DEFAULT_VERTICAL_INTERVAL);
        horizontalScrollSpeed = array.getInt(R.styleable.MarqueeView_horizontal_scroll_speed, DEFAULT_HORIZONTAL_SPEED);
        horizontalScrollInterval = array.getInt(R.styleable.MarqueeView_horizontal_scroll_interval, DEFAULT_HORIZONTAL_INTERVAL);
        horizontalLoopSpeed = array.getInt(R.styleable.MarqueeView_horizontal_loop_speed, DEFAULT_HORIZONTAL_LOOP_SPEED);
        contentColor = array.getColor(R.styleable.MarqueeView_content_text_color, Color.parseColor("#82786E"));
        contentTextSize = (int) array.getDimension(R.styleable.MarqueeView_content_text_size, Sp2Px(getContext(), 15));
        singleText = array.getString(R.styleable.MarqueeView_content_single_text);
...
//            Logger.e("textHeight=" + textHeight + " textWholeHeight=" + textWholeHeight);
    ...
    private void startHorizontalScroll() {
        ...
        horizontalScrollAnimator2 = ValueAnimator.ofFloat(0, 1);
        //在崩溃统计上看到值<0的bug
        ...
        horizontalScrollAnimator2.setDuration(horizontalScrollSpeed * xOffset / contentTextSize);
        horizontalScrollAnimator2.setInterpolator(new LinearInterpolator());
        horizontalScrollAnimator2.start();
        horizontalScrollAnimator2.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                float value = (float) animation.getAnimatedValue();
        ...
        horizontalScrollAnimator2.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
                super.onAnimationEnd(animation);
                isHorizontalRunning = false;
    ...
    private void startHorizontalLoop() {
        ...
        if (horizontalOriLeft) {
            horizontalScrollAnimator1 = ValueAnimator.ofFloat(0, 1);
        } else {
            horizontalScrollAnimator1 = ValueAnimator.ofFloat(0, -1);
        }
        if (horizontalScrollSpeed * xOffset / contentTextSize < 0) {
        ...
        horizontalScrollAnimator1.setDuration(horizontalLoopSpeed * xOffset / contentTextSize);
        horizontalScrollAnimator1.setInterpolator(new LinearInterpolator());
        horizontalScrollAnimator1.start();
        horizontalScrollAnimator1.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                float value = (float) animation.getAnimatedValue();
        ...
        horizontalScrollAnimator1.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
                super.onAnimationEnd(animation);
                isHorizontalRunning = false;
