...
public abstract class CameraSurfaceRenderer implements GLSurfaceView.Renderer, GLTextureView.Renderer {
    ...
    public void setCameraPreviewSize(int width, int height, int cameraDisplayOrientation) {
        Log.d(TAG, "setCameraPreviewSize() called with: width = [" + width + "], height = [" + height + "], cameraDisplayOrientation = [" + cameraDisplayOrientation + "]");
    ...
    public interface CustomTextureProcessor {
        ...
        /**
         * 重写此方法实现自定义纹理处理流程
         * @param textureId 待处理的纹理, {@link GLES20#GL_TEXTURE_2D} 类型, {@link GLES20#GL_RGBA} 像素格式
